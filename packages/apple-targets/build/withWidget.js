...
const withWidget = (config, props) => {
    ...
    if (props.icon) {
        if (typeof props.icon === "string") {
            ...
            const resolvedIcon = {};
            for (const key of ["light", "dark", "tinted"]) {
                const iconPath = props.icon[key];
                if (iconPath) {
                    resolvedIcon[key] = /https?:\/\//.test(iconPath)
                        ? iconPath
                        : path_1.default.join(props.directory, iconPath);
                }
            }
    ...
    const getIconName = () => {
        if (!props.icon)
            return undefined;
        const iconPath = typeof props.icon === "string" ? props.icon : props.icon.light;
        if (iconPath && path_1.default.extname(iconPath) === ".icon") {
            return path_1.default.basename(iconPath, ".icon");
        }
        return "AppIcon";
    };
